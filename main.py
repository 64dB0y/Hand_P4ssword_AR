...
with mp_hands.Hands(min_detection_confidence=0.5, min_tracking_confidence=0.5) as hands:
    while cap.isOpened():
        ...
        if current_cell_number:
            ...
            if previous_cell_number != current_cell_number:
                start_time = time.time()
                previous_cell_number = current_cell_number
                frame = draw_rectangle(frame, x, y, cell_width, cell_height, (0, 255, 255))
                input_allowed = True

            elif previous_cell_number == current_cell_number and input_allowed and time.time() - start_time >= 1 and time.time() - start_time < 2:
                frame = draw_rectangle(frame, x, y, cell_width, cell_height, (255, 0, 0))
            elif previous_cell_number == current_cell_number and input_allowed and time.time() - start_time >= 2:
                if ent_button:
                    print("ENT button pressed")
